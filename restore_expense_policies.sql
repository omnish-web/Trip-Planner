...
-- This ensures SELECT, INSERT, UPDATE, and DELETE are all correctly applied.
...
-- UPDATE: All trip members can update splits (the frontend UI handles restrictions)
CREATE POLICY "Splits updatable by members" ON expense_splits FOR UPDATE USING (
  expense_id IN (SELECT id FROM expenses WHERE trip_id IN (SELECT get_my_trip_ids()))
);

-- DELETE: All trip members can delete splits (needed when an expense is updated to remove a participant)
