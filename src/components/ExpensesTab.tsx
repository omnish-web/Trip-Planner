...
const ExpensesTab = React.memo(({
    ...
    // Unique categories from expenses (excluding Settlements)
    const allCategories = useMemo(() => {
        const cats = new Set<string>()
        expenses.filter(e => e.category !== 'Settlement' && e.title !== 'Settlement').forEach(e => cats.add(e.category))
        return Array.from(cats).sort()
    }, [expenses])

    // Group Expenses by Date (filtered by selectedCategory)
    const groupedExpenses = useMemo(() => {
        const groups: Record<string, any[]> = {}
        expenses
            .filter(e => e.category !== 'Settlement' && e.title !== 'Settlement')
        ...
        // Sort days descending; within each day sort by time descending (no time → last)
        const sorted = Object.entries(groups).sort((a, b) => new Date(b[0]).getTime() - new Date(a[0]).getTime())
        sorted.forEach(([, dayExpenses]) => {
                    ...
                    {allCategories.length > 1 && (
                        <div className="flex flex-wrap gap-2">
                            <button
                                ...
                                className={`px-3 py-1 rounded-full text-xs font-semibold border transition-all ${selectedCategory === 'All'
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <div>
                                                                <p className="font-bold text-gray-800 dark:text-white text-base flex items-center gap-2">
                                                                    <span className="truncate">{expense.title}</span>
                                                                    {expense.expense_time && (
                                                                        <span className="text-xs font-normal text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded font-mono shrink-0">
                                                                            {expense.expense_time}
                                                                        </span>
                                                                    )}
                                                                    {expense.attachment_url && (
                                                                        <a
                                                                            ...
                                                                            className="p-1 rounded-lg text-blue-500 hover:text-blue-600 hover:bg-blue-500/10 transition shrink-0"
                                                                ...
                                                                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                                                                    <span className="font-medium text-gray-700 dark:text-gray-300">
                                                                        {expense.expense_payers && expense.expense_payers.length > 1
                                                                            ? expense.expense_payers.map((p: any) => `${getParticipantName(p.participant_id)} (${currency} ${p.amount.toLocaleString()})`).join(', ')
                    ...
                    <div className="glass-panel p-5">
                        ...
                        <div className="space-y-3">
                            ...
                            {participants.filter(p => !p.parent_id).map(parent => {
                                const parentTotalSpent = expenses.filter(e => e.category !== 'Settlement' && e.title !== 'Settlement').reduce((sum, e) => {
                                    const payers = e.expense_payers || []
                                    if (payers.length > 0) {
                                        const participation = payers.find((p: any) => p.participant_id === parent.id)
                                        return sum + (participation ? participation.amount : 0)
                                    }
                                    return sum + (e.paid_by === parent.id ? e.amount : 0)
                                }, 0)
